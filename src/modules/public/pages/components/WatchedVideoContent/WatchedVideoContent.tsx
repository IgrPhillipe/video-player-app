...
import { useGetVideoById } from '@/api/videos';
import { useGetWatched } from '@/api/videos/queries/useGetWatched';
...
export const WatchedVideoContent = ({ userId }: WatchedVideoContentProps) => {
  ...
  const {
    ...
    params: { fromVideoId: parseInt(id as string), userId },
  });
  ...
  const handleNextVideo = useCallback(() => {
    if (autoplay) {
      router.push(`/assistidos/video/${filteredVideos?.[0]?.id}`);
    }
